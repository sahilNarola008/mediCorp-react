...
import {
    ...
    presentationHistoryByPresentationIdDataColumns
} from "@medicorp"
...
const usePresentation = () => {
    ...
    const [{ }, getPresentationHistoryByPresentationId] = useAxios(
        endpointConfig.presentationHistory.GetPresentationHistoryByPresentationId,
        { manual: true }
    )
    ...
    const presentationProductEditOptions = {
        ...
        onRowDelete: oldData =>
            new Promise((resolve, reject) => {
                ...
                setPresentationProductOrder(dataDelete.map(val => val.order))
                setPresentationProductId(dataDelete.map(val => val.productID))
                setModalFormResetKeys(['presentationProductMappings'])
                setModalContent(prevContent => ({
                    ...prevContent,
                    presentationProductMappings: {
    ...
    const handlePresentationClick = (presentationId) => {
        ...
        getPresentationHistoryByPresentationId(format(endpointConfig.presentationHistory.GetPresentationHistoryByPresentationId, presentationId)).then((res) => {
            ...
            setModalHeader({
                ...
                header: Strings.MENU_PRESENTATIONS_HISTORY_TITLE,
                modalWidth: "md",
            });
            setModalContent({
                processingStatus: {
                    ...
                    type: fieldTypes.table.type,
                    col: 12,
                    columns: presentationHistoryByPresentationIdColumns,
                    data: data ? data : [],
                    tableOptions: {
                        filtering: false,
                        selection: false,
    ...
    const { presentationColumns } = presentationDataColumns(handlePresentationClick);
    const { presentationHistoryByPresentationIdColumns } = presentationHistoryByPresentationIdDataColumns(handleDoctorNameClick, handleUserNameClick);

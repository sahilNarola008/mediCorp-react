...
export default function useDoctors() {
  ...
  const [{ data: doctors, loading: doctorsLoading }, refetchDoctors] = useAxios(
    endpointConfig.doctors.getAll
  );
  const [{ data: cities, loading: citiesLoading }, refetchCities] = useAxios(
    endpointConfig.city.getAll
  );
  const [{ data: states, loading: stattesLoading }, refetchStates] = useAxios(
    endpointConfig.state.getAll
  );
  const [{ data: countries, loading: countriesLoading }, refetchCountries] =
    useAxios(endpointConfig.country.getAll);
  ...
  const [{}, refetchDoctorsById] = useAxios(
    endpointConfig.doctors.getDoctorsById,
    { manual: true }
  );
  ...
  const handleActionClick = (
    ...
    setModalHeader({
      ...
      header:
        isEdit === true ? "Edit in existing Doctors" : "Create a new Doctors",
      modalWidth: "md",
    });
    setModalContent({
      firstName: {
        ...
        : [
            {
              ...
              action:
                isEdit === true
                  ? (data) => handleSubmit(data, true, rowData?.doctorId)

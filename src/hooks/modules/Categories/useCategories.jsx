...
function useCategories() {
  ...
  const [{}, refetchCategoriesById] = useAxios(
    endpointConfig.categories.getCategoriesById,
    { manual: true }
  );
        ...
        new Promise((resolve) => {
          confirm({ description: "Are you sure you want to delete?" }).then(
            () => {
              ...
              deleteCategories({
                url: format(
                  endpointConfig.categories.deleteCategoriesById,
                  rowData.categoryId
                ),
              })
                .then((res) => {
  ...
  const handleSubmit = (data, isEdit, id) => {
    ...
    response
      .then((res) => {
        ...
        logMessage({
          severity: !errorMessage ? statusType.success : statusType.error,
          msg:
            msg ??
            errorMessage ??
            message ??
        ...
        : [
            {
              ...
              action:
                isEdit === true
                  ? (data) => handleSubmit(data, true, rowData?.categoryId)
                  : (data) => handleSubmit(data, false, rowData?.id),
            },
          ]
    );
    setOpenDialog(true);
